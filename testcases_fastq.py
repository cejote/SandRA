...
def main(argv):
    ...
    for read in xrange(readcount):
        ...
        lengthvar=0 #random.randint(-readlendelta/2, readlendelta/2)

        adapt=random.choice(adapters)
        
        
        adaptstart=0 #random.randint(0, len(adapt)/5)
        adaptend=20 # random.randint(len(adapt)-(len(adapt)/5), len(adapt))
